...
# Andrew - you should be able to put the matched dataset with the temperature dataset into this and get the dlnms...this is the chaotic file where I made a new copy of everything to change every single parameter.

# I'm going to send you the matched file so you can run this without rerunning all of the data. Upload warm_season_matched_data_blocks_2022.csv and then run the cells with this in file #4 and then come back and do the analysis

# also daniel wants me to try the cumulative thing and to try with a quasipoisson instead of clogit...I did this in one line below. 
...
analyze_data <- function(data, lag_var, start_year, end_year, plot_fix_temp, plot_fix_lag) {
  
  # Filter data based on specified block and opioid contribution
  dataf <- data %>%
    filter(block %in% c(1, 2, 3, 4, 5, 6, 7),
           opioid_contributing == "yes")
  
  # Convert Date column to Date type
  dataf$Date <- as.Date(dataf$Date)
  
  # Create final dataframe by binding rows with data_aggregate
  final_dataframe <- bind_rows(dataf, data_aggregate)
  
  # Extract year from Date and filter based on start_year and end_year
  ...
  final_dataframe <- final_dataframe %>%
    ...
    group_by(county) %>%
    mutate(
      lag0 = !!rlang::sym(lag_var),
      lag1 = lag(!!rlang::sym(lag_var), 1),
      lag2 = lag(!!rlang::sym(lag_var), 2),
      lag3 = lag(!!rlang::sym(lag_var), 3),
      lag4 = lag(!!rlang::sym(lag_var), 4),
      lag5 = lag(!!rlang::sym(lag_var), 5),
      lag6 = lag(!!rlang::sym(lag_var), 6)
    ) %>%
    ungroup()
  
  # Create the exposure history matrix
  Qdata <- final_dataframe %>%
  dplyr::select(lag0:lag6)
  
  # Creating a crossbasis matrix
  range <- range(final_dataframe[[lag_var]],na.rm=T)
  nknots <- 4
  nlagknots <- 2
  ...
  # Regression model
  mnest <- clogit(binary ~ cbnest + strata(uniqueID), final_dataframe, method = "exact")
  
  # Predicting specific effect summaries
  
  cenvalue=median(final_dataframe[[lag_var]])
  range <- range(final_dataframe[[lag_var]],na.rm=T)
  pnest <- crosspred(cbnest, mnest, cen=cenvalue, at=round(range[1]):round(range[2]))
  
 # Plotting
  plot1 <- plot(pnest, zlab = "OR", xlab = paste(lag_var), ylab = "Lag (days)")
  plot2 <- plot(pnest, var = plot_fix_temp, ylab = paste("OR for", lag_var, plot_fix_temp), xlab = "Lag (days)", xlim = c(0, 6))
  plot3 <- plot(pnest, lag = plot_fix_lag, ylab = paste("OR at lag", plot_fix_lag), xlab = paste(lag_var))

  # Checking AIC
  models <- list(mnest)
  aic <- aictab(cand.set = models)

  return(list(plot1 = plot1, plot2 = plot2, plot3 = plot3, aic = aic))
}


result <- analyze_data(warm_season_matched_data_blocks, lag_var = "HeatIndex", start_year = 2000, end_year = 2010, plot_fix_temp=160, plot_fix_lag=4)
...
#write.csv(ci_df, "~/Thesis Work/Thesis Files/tables/warm_overall_CI.csv")
...
# warm_season_matched_data_blocks_edited <- warm_season_matched_data_blocks_edited %>%
#                     filter(cocaine_contributing == "yes")
# 
# warm_season_matched_data_blocks_edited <- warm_season_matched_data_blocks_edited %>%
#                     filter(other_stimi_contributing == "no")
...
basis.temp <- crossbasis(data$MeanTemperature, lag = 15, vartype="ns", varknots=ktemp,
...
cb1.temp <- crossbasis(data$MeanTemperature, lag=3, argvar=list(df=5),
...
cbnest <- crossbasis(data$MeanTemperature, lag = 15, vartype="ns", varknots=ktemp,
cenvalue=median(data$MeanTemperature,na.rm=T), lagtype="ns", lagknots=klag,maxlag=15)
...
cb3.temp <- crossbasis(data$MeanTemperature, lag=30, argvar=list(fun="bs",
...
# trying to create a matrix of exposure histories
head(data,1000)
...
# # this makes a long format and doesnt work
# # Function to calculate lag temperature values for each row
# calculate_lag_temperatures <- function(temp_values, lag) {
#   lagged_temps <- zoo::rollapply(temp_values, width = lag + 1, FUN = mean, align = "right", na.rm = TRUE, fill = NA)
#   lagged_temps <- tail(lagged_temps, num_lag_days)
#   return(lagged_temps)
# }
# 
# # Use purrr::pmap_dfr() to apply the function row-wise and bind rows
# lag_temp_df <- data %>%
#   filter(!is.na(uniqueID)) %>%
#   pmap_dfr(~ tibble(lag_temp = calculate_lag_temperatures(data$MeanTemperature, 0:14)))
# 
...
cbcases_test <- crossbasis(Qdata_cases, lag=14, argvar=list("lin"),
